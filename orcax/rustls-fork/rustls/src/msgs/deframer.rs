...
impl MessageDeframer {
    ...
    /// Extract all buffered bytes from the deframer. Used when transitioning
    /// from TLS to raw TCP (DeepSea splice) — bytes read from the socket but
    /// not yet processed as TLS records need to be prepended to raw reads.
    pub fn take_pending(&mut self) -> Vec<u8> {
        if self.used == 0 {
            return Vec::new();
        }
        let data = self.buf[..self.used].to_vec();
        self.used = 0;
        data
    }
