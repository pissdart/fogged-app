...
impl<Data> ConnectionCommon<Data> {
    /// Check if the internal deframer has buffered encrypted bytes from the socket
    /// that haven't been processed into complete TLS records yet.
    /// Used to determine if kTLS handoff is safe (only safe when this returns false).
    pub fn deframer_has_pending(&self) -> bool {
        self.core.message_deframer.has_pending()
    }

    /// Extract all buffered bytes from the deframer. These are bytes that were
    /// read from the socket but not yet processed as TLS records. Used when
    /// transitioning from TLS to raw TCP (e.g., DeepSea splice).
    pub fn deframer_take_pending(&mut self) -> Vec<u8> {
        self.core.message_deframer.take_pending()
    }
