...
import 'package:fogged/core/model/directories.dart';
...
import 'package:fogged/features/connection/model/connection_failure.dart';
...
import 'package:fogged/foggedcore/generated/v2/hcommon/common.pb.dart';
import 'package:fogged/foggedcore/generated/v2/hcore/hcore.pb.dart';
import 'package:fogged/foggedcore/generated/v2/hcore/hcore_service.pbgrpc.dart';
import 'package:fogged/foggedcore/init_signal.dart';
import 'package:fogged/singbox/model/singbox_config_option.dart';
import 'package:fogged/features/log/model/log_level.dart' as config_log_level;
import 'package:fogged/singbox/model/core_status.dart';
import 'package:fogged/singbox/model/warp_account.dart';

import 'package:fogged/foggedcore/core_interface/core_interface_wrapper_stub.dart'
    if (dart.library.io) 'package:fogged/foggedcore/core_interface/core_interface_wrapper.dart';
import 'package:fogged/utils/custom_loggers.dart';
import 'package:fogged/utils/platform_utils.dart';
import 'package:hooks_riverpod/hooks_riverpod.dart';
import 'package:loggy/loggy.dart' as loggyl;
import 'package:riverpod_annotation/riverpod_annotation.dart';
import 'package:rxdart/rxdart.dart';

class FoggedCoreService with InfraLogger {
  FoggedCoreService(this.ref);
  final Ref ref;

  // CoreFoggedCoreService() {}
  ...
  Future<void> init() async {
    await setup()
        ...
        .map((_) {
          loggy.info("Fogged-core setup done");
  ...
  TaskEither<String, Unit> changeOptions(SingboxConfigOption options) {
    return TaskEither(() async {
      ...
      try {
        final res = await core.fgClient.changeFoggedSettings(
          ChangeFoggedSettingsRequest(foggedSettingsJson: jsonEncode(options.toJson())),
        );
        if (res.messageType != MessageType.EMPTY) return left("${res.messageType} ${res.message}");
        await core.bgClient.changeFoggedSettings(
          ChangeFoggedSettingsRequest(foggedSettingsJson: jsonEncode(options.toJson())),
        );
  ...
  TaskEither<ConnectionFailure, Unit> start(String path, String name, bool disableMemoryLimit) {
    return TaskEither(() async {
      ...
      //   await core.bgClient.changeFoggedSettings(
      //     ChangeFoggedSettingsRequest(
      //       foggedSettingsJson: jsonEncode(latestOptions!.toJson()),
      //     ),
      //   );
      // }
