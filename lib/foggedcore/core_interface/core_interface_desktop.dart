...
import 'package:fogged/gen/fogged_core_generated_bindings.dart';
import 'package:fogged/foggedcore/core_interface/core_interface.dart';
import 'package:fogged/foggedcore/core_interface/mtls_channel_cred.dart';
...
import 'package:fogged/foggedcore/generated/v2/hello/hello.pb.dart';
import 'package:fogged/foggedcore/generated/v2/hello/hello_service.pbgrpc.dart';
...
final _logger = Loggy('FoggedCoreFFI');
...
class CoreInterfaceDesktop extends CoreInterface with InfraLogger {
  static final FoggedCoreNativeLibrary _box = _gen();

  static FoggedCoreNativeLibrary _gen() {
    String fullPath = "";
    if (Platform.environment.containsKey('FLUTTER_TEST')) {
      fullPath = "fogged-core";
    }
    if (Platform.isWindows) {
      fullPath = p.join(fullPath, "fogged-core.dll");
    } else if (Platform.isMacOS) {
      fullPath = p.join(fullPath, "fogged-core.dylib");
    } else {
      fullPath = p.join(fullPath, "fogged-core.so");
    }

    _logger.debug('fogged-core native libs path: "$fullPath"');
    ...
    return FoggedCoreNativeLibrary(lib);
  }
